...
function exportPriceReportPDF() {
  ...
  win.document.write(`<!DOCTYPE html><html><head><title>AI Price Report – ${label}</title>
    ...
    <div class="footnote">AI market estimate generated by Claude based on Canadian retail pricing for ${vehicle.condition === 'new' || Number(vehicle.year) >= new Date().getFullYear() ? 'new vehicles of the same year' : 'used vehicles of the same year and trim'}. Estimates reflect typical market conditions and are not a guarantee of resale value. Generated ${new Date().toLocaleDateString('en-CA', { year: 'numeric', month: 'long', day: 'numeric' })}.</div>
  </body></html>`);
  win.document.close();
  win.focus();
